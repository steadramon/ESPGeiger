...
static const EGPref LED_PREF_ITEMS[] = {
  ...
  {"pulse_ms",    LD_L_PUL, LD_H_PUL, "2",     nullptr, 1,    50,    0, EGP_UINT, EGP_ADVANCED},
#else
#if !(GEIGER_IS_TEST(GEIGER_TYPE) && defined(ESP8266))
...
#endif
  {"pulse_ms",    LD_L_PUL, LD_H_PUL, "20",    nullptr, 1,    50,    0, EGP_UINT, EGP_ADVANCED},
#if !(GEIGER_IS_TEST(GEIGER_TYPE) && defined(ESP8266))
...
void LedPrefs::on_prefs_loaded() {
  ...
  LedSignal::setBlipEngineConfig(
    engine_mode,
    (uint16_t)(EGPrefs::getUInt("led", "pulse_ms") * 1000),
    3500, 1, fadeIdx,
    0
  );
#ifndef ESPGEIGER_HW
