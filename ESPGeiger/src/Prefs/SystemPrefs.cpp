...
static const EGPref INPUT_PREF_ITEMS[] = {
  ...
  {"fast_cpm",    "Fast CPM",    "5s window instead of 60s (reboot to apply)", "0", nullptr, 0, 1, 0, EGP_BOOL, 0},
#endif
...
void InputPrefs::on_prefs_loaded() {
  ...
  gcounter.set_fast_cpm(EGPrefs::getBool("input", "fast_cpm"));
#endif
#ifndef RXPIN_BLOCKED
...
void InputPrefs::on_prefs_saved() {
  ...
  if (gcounter.get_fast_cpm() != EGPrefs::getBool("input", "fast_cpm")) need_reboot = true;
#endif
  on_prefs_loaded();
