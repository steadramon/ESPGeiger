...
private:
  ...
  // s_tick sets pending bits; loop() services them so socket, mDNS and
  // UDP send cost stays out of tick_max.
  ...
  static constexpr uint8_t BIND     = 1 << 4;
  static constexpr uint8_t REBIND   = 1 << 5;
  uint8_t   _pending = 0;
