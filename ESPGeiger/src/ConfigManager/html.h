...
static const char picographJS[] PROGMEM = R"JS(function byID(t){return document.getElementById(t)}function cVID(t,s){const i=[];for(let e=0;e<t.length;e++)i[e]=s+t[e].replace(" ","")+"value";return i}function cLeg(t,s,i,e){const h=`<span>${i}</span>`,n=":"==i.at(-1)?`<span id="${e}"></span>`:"";byID(t).innerHTML+=`<div style="display: inline-block;"><svg width="10" height="10"><rect width="10" height="10" style="fill: ${s}"/></svg> ${h} ${n}</div>`}class Graph{constructor(t,s,i,e,h,n,a=0,l=!1,o=!1,r=5,c=1,m=2){const d= cVID(s,t);this.initE(t,e,d),this.sCfg(s,i,n,a,l,o,r,c,m),this.tscount=0,this.vlcount=0,this.rset(h),""===byID(e).innerHTML&&this.cLgs()}initE(t,s,i){this.canvas=byID(t),this.ctx=this.canvas.getContext("2d"),this.sWH(),this.lID=s,this.vIDs=i}sCfg(t,s,i,e,h,n,a,l,o){this.labels=t,this.unit=s,this.vlFrq=l,this.vlines=h,this.tms=n,this.noLabels=t.length,this.colors=cA(this.noLabels),this.scCfg(i,e,a,o)}scCfg(t,s,i,e){this.mv=t,this.lv=s,this.sstps=i,this.auSM=e,0==[1,2].includes(e)&&(this.auSM=0,!1===Number.isFinite(s)&&(this.lv=0),!1===Number.isFinite(t)&&(this.mv=100))}rset(t){this.intSz=t*this.cssScl,this.nPF=this.width/this.intSz,this.nPoints=Math.round(this.nPF)+1,this.points=eA2D(this.noLabels,this.nPoints),this.tsA=eA(this.nPoints,""),this.fontSize=null,this.hstep=null,this.sstp=null}cLgs(){byID(this.lID).innerHTML="";for(let t=0;t<this.noLabels;t++)cLeg(this.lID,this.colors[t],this.labels[t]+":",this.vIDs[t])}uCfg(t,s,i,e,h=0,n=!1,a=!1,l=5,o=1,r=1){this.clr(),this.sCfg(t,s,e,h,n,a,l,o,r),this.rset(i),this.cLgs()}uMM(t){t<this.lv&&(this.lv=t),t>this.mv&&(this.mv=t);let s=Math.abs(this.mv-this.lv),i=Math.ceil(Math.max.apply(Math,this.points.flat().filter(Number.isFinite)))+.05*s,e=Math.floor(Math.min.apply(Math,this.points.flat().filter(Number.isFinite)))-.05*s;if(2==this.auSM)return this.mv=i;t>this.mv-.05*s&&(this.mv=i),t<this.lv+.05*s&&(this.lv=e)}upPts(t){for(let s=0;s<this.noLabels;s++)this.auSM>0&&this.uMM(t[s]),this.points[s]=sAL(this.points[s],t[s])}upLgs(t){for(let s=0;s<this.noLabels;s++)byID(this.vIDs[s]).innerHTML=t[s].toFixed(2)+" "+this.unit}uTS(){const t=gTS();this.tsA=sAL(this.tsA,t)}drawVL(){this.vlcount++;for(let t=this.nPoints-this.vlcount;t>=0;t-=this.vlFrq){const s=t*this.intSz;this.ctx.beginPath(),this.ctx.moveTo(s,0),this.ctx.lineTo(s,this.height),this.ctx.strokeStyle="#DDD",this.ctx.stroke()}this.vlcount==this.vlFrq&&(this.vlcount=0)}clr(){this.ctx.clearRect(0,0,this.width,this.height)}sWH(){this.cssScl=window.devicePixelRatio,this.canvas.width=this.canvas.clientWidth*this.cssScl,this.canvas.height=this.canvas.clientHeight*this.cssScl,this.width=this.ctx.canvas.width,this.height=this.ctx.canvas.height}sIWH(){this.intSz=this.width/this.nPF,this.ctx.lineWidth=2*this.cssScl}setStp(){this.hstep=this.height/this.sstps,this.sstp=(this.mv-this.lv)/this.sstps,this.fontSize=Math.min(.5*this.hstep,15*this.cssScl),this.ctx.font=this.fontSize+"px monospace"}drawHL(){let t=document.createElement("textarea");t.innerHTML=this.unit;for(let s=1;s<=this.sstps;s++){const i=this.height-s*this.hstep,e=2,h=this.fontSize+2*this.cssScl;this.ctx.fillText((s*this.sstp+this.lv).toFixed(2)+" "+t.value,e,i+h),this.ctx.beginPath(),this.ctx.moveTo(0,i),this.ctx.lineTo(this.width,i),this.ctx.strokeStyle="#DDD",this.ctx.stroke()}}dTS(){const t=this.ctx.measureText((this.sstps*this.sstp).toFixed(2)).width,s=Math.floor(t/this.intSz+1)+1;this.tscount++;for(let t=this.nPoints-this.tscount;t>=s;t-=this.vlFrq){const s=t*this.intSz,i=this.fontSize+2*this.cssScl,e=this.ctx.measureText(this.tsA[t]).width+4*this.cssScl;this.ctx.rotate(Math.PI/2),this.ctx.fillText(this.tsA[t],this.height-e,-s+i),this.ctx.stroke(),this.ctx.rotate(-Math.PI/2)}this.tscount==this.vlFrq&&(this.tscount=0)}draw(){for(let t=this.noLabels-1;t>=0;t--)for(let s=this.nPoints-1;s>0;s--){const i=s*this.intSz,e=(s-1)*this.intSz,h=sclInv(this.points[t][s],this.lv,this.mv,this.height),n=sclInv(this.points[t][s-1],this.lv,this.mv,this.height);this.ctx.beginPath(),this.ctx.moveTo(i,h),this.ctx.lineTo(e,n),this.ctx.strokeStyle=this.colors[t],this.ctx.stroke()}}update(t){this.upPts(t),this.upLgs(t),this.uTS(),this.clr(),this.sWH(),this.sIWH(),this.setStp(),this.vlines&&this.drawVL(),this.drawHL(),this.tms&&this.dTS(),this.draw()}}function gTS(){const t=new Date;return t.toLocaleTimeString()}function sclInv(t,s,i,e){return(1-(t-s)/(i-s))*e}function sAL(t,s){return t.shift(),t.push(s),t}function eA2D(t,s,i){return Array.from({length:t},(t=>Array.from({length:s},(t=>i))))}function eA(t,s){return Array.from({length:t},(t=>s))}const cls=["#D35","#090","#00F","#F0F","#933","#009","#099","#999"];function cA(t){const s=[];for(let i=0;i<t;i++)s[i]=cls[i%cls.length];return s})JS";