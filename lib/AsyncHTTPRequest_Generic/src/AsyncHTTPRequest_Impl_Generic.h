...
bool  AsyncHTTPRequest::open(const char* method, const char* URL)
{
  ...
  // Stack buffer: host (RFC 1035 cap 253) + ':' + uint16 port + NUL.
  char hostName[270];
  snprintf(hostName, sizeof(hostName), "%s:%d", _URL->host, _URL->port);
...
bool AsyncHTTPRequest::send(const String& body)
{
  ...
  char clen[12];
  snprintf(clen, sizeof(clen), "%u", (unsigned)body.length());
  _addHeader("Content-Length", clen);
...
bool  AsyncHTTPRequest::send(const char* body)
{
  ...
  char clen[12];
  snprintf(clen, sizeof(clen), "%u", (unsigned)strlen(body));
  _addHeader("Content-Length", clen);
...
bool   AsyncHTTPRequest::_buildRequest()
{
  ...
  _request->write(_methodStr(_HTTPmethod));
